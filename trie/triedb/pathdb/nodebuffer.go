...
func (m *memtable) merge(m1 *memtable) (*memtable, error) {
	...
	res.size = m.size + m1.size
	res.layers = m.layers + m1.layers
	res.limit = m.size
...
func (b *nodebuffer) flush(db ethdb.KeyValueStore, clean *fastcache.Cache, id uint64, force bool) error {
	...
	if force {
		for {
			if atomic.LoadUint64(&b.backup.immutable) == 1 {
				time.Sleep(time.Duration(DefaultBackgroundFlushInterval))
	...
	go func() {
		for {
			...
			err := b.backup.flush(db, clean, id-b.current.layers)
			if err == nil {
				return
			}
			log.Error("failed to flush background memory table to disk", "error", err)
		}
	}()
	return nil
}
